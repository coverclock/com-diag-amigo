...
static const uint8_t PIN[] PROGMEM = {
		...
		// Gratefully adapted from variants/mega/pins_arduino.h in
		// Arduino 1.0.
		GPIO::PIN_E0,
		GPIO::PIN_E1,
		GPIO::PIN_E4,
		...
		// Gratefully adapted from variants/standard/pins_arduino.h in
		// Arduino 1.0.
		GPIO::PIN_D0,
		GPIO::PIN_D1,
		GPIO::PIN_D2,
...
GPIO::Pin GPIO::arduino(uint8_t id) {
	return static_cast<Pin>((id < countof(PIN)) ? pgm_read_byte(&(PIN[id])) : ~0);
}
